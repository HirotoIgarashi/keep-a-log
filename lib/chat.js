  ...
  connect : function ( server ) {
    ...
    io
      ...
      .on( 'connection', function ( socket ) {
        ...
        //      クライアントがユーザを関連付けられるようにするが、
        //      MongoDBには格納しない。
        //  ユーザをオンラインとしてマークし、「adduser」メッセージを
        //  発行した。
        //    クライアントを含めた全クライアントに更新された
        //    オンラインユーザリストを配信する。
        //
        ...
        //  オンラインではない場合、「user has gone offline」という
        //  メッセージを送信者に送信する。
        //
        socket.on( 'updatechat',    function ( chat_map ) { 
          if ( Object.prototype.hasOwnProperty.call(chatterMap, chat_map.dest_id ) ) {
        ...
        // 引数: 1つのavtr_mapオブジェクトavtr_mapは以下のプロパティを
        //       持つべき。
        //  person_id : 更新するユーザアバターのID
        //  css_map   : 上端、左端、背景色のcssマップ
        //
        // 動作:
        //  このハンドラはMongoDBのエントリを更新し、全クライアントに
        //  修正したユーザリストを配信する。
        //
        socket.on( 'updateavatar',  function ( avtr_map ) {
          crud.update(
            ...
            function (/* result_map */) { emitUserList( io ); }
          );
        });
